package com.github.AlexBogdanov.p02_ExtendedDatabase;
...
public class Database {
    ...
    public void add(Person person) throws OperationNotSupportedException {
        ...
        Person fetchedPerson = Arrays.stream(this.elements)
    ...
    private void setElements(Person... elements) throws OperationNotSupportedException {
        Person personWithInvalidId = Arrays.stream(elements)
            .filter(Objects::nonNull)
            .filter(p -> p.getId() < 0)
            .findFirst()
            .orElse(null);

        if (personWithInvalidId != null) {
            throw new OperationNotSupportedException();
        }
